...
a = '轮船'
maxn = '20'
#data1 = test_graph.run("Match (n:Person{name: $str})-[r:CALL]-(end:Person) return r.count", str=a).data()
#print(data1)
#data = test_graph.run("Match (n:Person{name: $str})-[r:CALL]-(end:Person) return r.value, "
                       #"n.name,end.name order by r.value desc limit " + maxn, str=a).data()
data = test_graph.run("Match (n:Entity{name: $str})-[r:REL]-(end:Entity) return r.value, r.rel, r.item, r.content, "
                           "n.name,end.name order by r.value desc", str=a).data()
print(data)
# tdata = []
